...
caffemodel = '../12net/models_300/solver_iter_500000.caffemodel'
...
neg_save_dir  = "24/negative_from12"
pos_save_dir  = "24/positive_from12"
part_save_dir = "24/part_from12"
image_size = 24
f1 = open('24/pos_24_from12.txt', 'w')
f2 = open('24/neg_24_from12.txt', 'w')
f3 = open('24/part_24_from12.txt', 'w')
...
for annotation in annotations:
    ...
    for box in rectangles:
        ...
        # crop image patch
        x_left, y_top, x_right, y_bottom, _ = np.array(box).astype(int)
        ...
        if np.max(Iou) < 0.3:
            ...
            f2.write("%s/negative_from12/%s"%(image_size, n_idx) + ' 0\n')
            ...
            if np.max(Iou) >= 0.65:
                ...
                f1.write("%s/positive_from12/%s"%(image_size, p_idx) + ' 1 %.2f %.2f %.2f %.2f\n'%(offset_x1, offset_y1, offset_x2, offset_y2))
                cv2.imwrite(save_file, resized_im)
                p_idx += 1

            elif np.max(Iou) >= 0.4:
                save_file = os.path.join(part_save_dir, "%s.jpg"%d_idx)
                f3.write("%s/part_from12/%s"%(image_size, d_idx)     + ' -1 %.2f %.2f %.2f %.2f\n'%(offset_x1, offset_y1, offset_x2, offset_y2))
                cv2.imwrite(save_file, resized_im)
                d_idx += 1
